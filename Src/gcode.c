...
uint8_t gc_execute_line(char *line) {
	...
	while (line[char_counter] != 0) { // Loop until no more g-code words in line.
		...
		case 'G':
			...
			case 53:
				...
				case 30:
					switch (mantissa) {
					case 0:
						gc_block.non_modal_command = NON_MODAL_GO_HOME_1;
						break;  // G30
					case 10:
						gc_block.non_modal_command = NON_MODAL_SET_HOME_1;
						break; // G30.1
					default:
						FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND)
						; // [Unsupported G30.x command]
					}
					mantissa = 0; // Set to zero to indicate valid non-integer G command.
					break;
				case 53:
			...
			case 80:
				...
				case 38:
					...
					break;
				case 80:
					gc_block.modal.motion = MOTION_MODE_NONE;
					break; // G80
				}
				break;
			case 17:
			...
			case 91:
				if (mantissa == 0) {
					...
					if (int_value == 90) {
						gc_block.modal.distance = DISTANCE_MODE_ABSOLUTE;
					} // G90
					else {
			...
			case 94:
				...
				if (int_value == 93) {
					gc_block.modal.feed_rate = FEED_RATE_MODE_INVERSE_TIME;
				} // G93
				else {
			...
			case 21:
				...
				if (int_value == 20) {
					gc_block.modal.units = UNITS_MODE_INCHES;
				}  // G20
				else {
					gc_block.modal.units = UNITS_MODE_MM;
				} // G21
				break;
			case 40:
		...
		case 'M':
			...
			case 30:
				...
				case 2:
				case 30:
					gc_block.modal.program_flow = PROGRAM_FLOW_COMPLETED;
		...
		default:
			...
			if (bit(word_bit)
					& (bit(WORD_F) | bit(WORD_N) | bit(WORD_P) | bit(WORD_T)
							| bit(WORD_S))) {
				if (value < 0.0) {
	...
	memcpy(coordinate_data, gc_state.coord_system,
...
//		if (!settings_read_coord_data(coord_select, parameter_data)) {
//			FAIL(STATUS_SETTING_READ_FAIL);
//		} // [EEPROM read fail]
		...
		for (idx = 0; idx < N_AXIS; idx++) { // Axes indices are consistent, so loop may be used.
			...
			if (bit_istrue(axis_words, bit(idx))) {
				if (gc_block.values.l == 20) {
					...
					parameter_data[idx] = gc_state.position[idx]
							- gc_state.coord_offset[idx]
							- gc_block.values.xyz[idx];
					if (idx == TOOL_LENGTH_OFFSET_AXIS) {
						parameter_data[idx] -= gc_state.tool_length_offset;
					}
				} else {
		...
		for (idx = 0; idx < N_AXIS; idx++) { // Axes indices are consistent, so loop may be used.
			if (bit_istrue(axis_words, bit(idx))) {
				gc_block.values.xyz[idx] = gc_state.position[idx]
						- coordinate_data[idx] - gc_block.values.xyz[idx];
				if (idx == TOOL_LENGTH_OFFSET_AXIS) {
			...
			if (axis_words) {
				for (idx = 0; idx < N_AXIS; idx++) { // Axes indices are consistent, so loop may be used to save flash space.
					if (bit_isfalse(axis_words, bit(idx))) {
						...
						if (gc_block.non_modal_command
								!= NON_MODAL_ABSOLUTE_OVERRIDE) {
							// Apply coordinate offsets based on distance mode.
							if (gc_block.modal.distance
									== DISTANCE_MODE_ABSOLUTE) {
								gc_block.values.xyz[idx] += coordinate_data[idx]
										+ gc_state.coord_offset[idx];
								if (idx == TOOL_LENGTH_OFFSET_AXIS) {
									gc_block.values.xyz[idx] +=
...
//			if (!settings_read_coord_data(SETTING_INDEX_G28, parameter_data)) {
//				FAIL(STATUS_SETTING_READ_FAIL);
//			}
			break;
...
//			if (!settings_read_coord_data(SETTING_INDEX_G30, parameter_data)) {
//				FAIL(STATUS_SETTING_READ_FAIL);
//			}
			break;
		...
		if (gc_block.modal.motion == MOTION_MODE_SEEK) {
			...
			case MOTION_MODE_CCW_ARC:
				...
				if (value_words & bit(WORD_R)) { // Arc Radius Mode
					...
					if (gc_check_same_position(gc_state.position,
							gc_block.values.xyz)) {
					...
					float h_x2_div_d = 4.0 * gc_block.values.r
							* gc_block.values.r - x * x - y * y;
					...
					bit_false(value_words,
							(bit(WORD_I) | bit(WORD_J) | bit(WORD_K)));
...
#else
				// TODO: My implementation here.
//				mc_line(gc_block.values.xyz, gc_state.feed_rate,
//						gc_state.modal.feed_rate);
#endif
				break;
			case MOTION_MODE_CW_ARC:
#ifdef USE_LINE_NUMBERS
...
//						gc_state.feed_rate, gc_state.modal.feed_rate, axis_0, axis_1, axis_linear, true, gc_state.line_number);
				...
				(void)axis_linear;
				// TODO: My implementation here.
//				mc_arc(gc_state.position, gc_block.values.xyz,
//						gc_block.values.ijk, gc_block.values.r,
//						gc_state.feed_rate, gc_state.modal.feed_rate, axis_0,
//						axis_1, axis_linear, true);
#endif
				break;
			case MOTION_MODE_CCW_ARC:
#ifdef USE_LINE_NUMBERS
...
#else
				// TODO: My implementation here.
				// mc_probe_cycle(gc_block.values.xyz, gc_state.feed_rate,
//						gc_state.modal.feed_rate, false, false);
#endif
				break;
			case MOTION_MODE_PROBE_TOWARD_NO_ERROR:
#ifdef USE_LINE_NUMBERS
				...
				// TODO: My implementation here.
//				mc_probe_cycle(gc_block.values.xyz, gc_state.feed_rate,
//						gc_state.modal.feed_rate, false, true);
#endif
				break;
			case MOTION_MODE_PROBE_AWAY:
#ifdef USE_LINE_NUMBERS
...
//				mc_probe_cycle(gc_block.values.xyz, gc_state.feed_rate, gc_state.modal.feed_rate, true, false, gc_state.line_number);
				...
				// TODO: My implementation here.
//				mc_probe_cycle(gc_block.values.xyz, gc_state.feed_rate,
//						gc_state.modal.feed_rate, true, false);
#endif
				break;
			case MOTION_MODE_PROBE_AWAY_NO_ERROR:
#ifdef USE_LINE_NUMBERS
...
//				// TODO: My implementation here.
//				mc_probe_cycle(gc_block.values.xyz, gc_state.feed_rate,
//						gc_state.modal.feed_rate, true, true);
#endif
			}
...
//					FAIL(STATUS_SETTING_READ_FAIL);
//				}
//				memcpy(gc_state.coord_system, coordinate_data,
