...
static void dfa_get_matching_node_subroutine(const struct dfa_node_t *curr_node,
                                             const char *const s, struct dfa_node_t **const res) {
  ...
  const struct dfa_node_t *kNextNode =
    curr_node->next[first_symbol_ascii_value];
  if (kNextNode == NULL) {
    *res = NULL;
    return;
  }
...
static const char *no_autocompletion_variants_available =
		"no autocompletion variants available";
static const char *smth_went_wrong = "smth went wrong";

#endif

const char* dfa_get_next_autocompletion_variant(const struct dfa_t *const dfa,
                                                const char *const s) {
  ...
  if (matching_node == NULL) {  // failed to find a match
    ...
    char *const reason = calloc(101, sizeof(char));
		if (dfa->root.next[(size_t) 'l'] == NULL) {
			...
			sprintf(reason, "node %d has %d autocompletion variants",
  ...
  const char *const res =
    matching_node->autocompletion_variants[matching_node->autocompletion_offset];
  matching_node->autocompletion_offset++;
  matching_node->autocompletion_offset %=
    matching_node->autocompletion_variants_count;
#ifdef DBG_TOOLS
  if (res == NULL) {
...
static struct dfa_node_mutable_t* dfa_node_mutable_create_empty() {
  struct dfa_node_mutable_t *new_node = malloc(
    sizeof(struct dfa_node_mutable_t));
  if (new_node == NULL) {
    abort();
  }
...
static struct dfa_node_mutable_t* dfa_node_mutable_append(
  struct dfa_node_mutable_t **const node, const char *const command) {
  if (*node == NULL) {
    abort();
  }
  ...
  (*node)->autocompletion_variants[(*node)->autocompletion_variants_count] =
    calloc(strlen(command) + 1, sizeof(char));
  strcpy(
    (*node)->autocompletion_variants[(*node)->autocompletion_variants_count],
    command);
...
static struct dfa_node_mutable_t* dfa_node_mutable_create(
  const char *const command) {
  struct dfa_node_mutable_t *empty_node = dfa_node_mutable_create_empty();
  return dfa_node_mutable_append(&empty_node, command);
}

static struct dfa_node_mutable_t* dfa_node_mutable_append_or_create(
  struct dfa_node_mutable_t **const node, const char *const command) {
  if (*node == NULL) {
...
static struct dfa_node_mutable_t* dfa_insert_everything_but_last_symbol_of_token(
  struct dfa_node_mutable_t *last_matching_node_ptr,
  const char *const token) {
  for (size_t j = 0; token[j + 1] != '\0'; ++j) {
    last_matching_node_ptr = dfa_node_mutable_append_or_create(
      &(last_matching_node_ptr->next[(size_t) token[j]]),
      token + j + 1);
  }
  return last_matching_node_ptr;
}

static void dfa_run_callback_on_each_sibling(
  const struct dfa_node_t *const node,
...
static void dfa_make_autocompletion_variants_unique(
  const struct dfa_node_t *const node) {
  if (node->autocompletion_variants_count == 0) {
    return;
  }

  struct dfa_node_mutable_t *const mut_node =
    (struct dfa_node_mutable_t*) node;

  qsort(mut_node->autocompletion_variants,
        mut_node->autocompletion_variants_count, sizeof(char*), cmp_str);
  size_t unique_count = 1;
  size_t *unique_ids = calloc(1, sizeof(size_t));
  for (size_t i = 1; i < mut_node->autocompletion_variants_count; ++i) {
    if (strcmp(mut_node->autocompletion_variants[i - 1],
               mut_node->autocompletion_variants[i]) != 0) {
      ++unique_count;
  ...
  for (size_t i = 1; i < unique_count; ++i) {
    strcpy(mut_node->autocompletion_variants[i],
           mut_node->autocompletion_variants[unique_ids[i]]);
  }
  mut_node->autocompletion_variants = realloc(
    mut_node->autocompletion_variants, sizeof(size_t) * unique_count);
...
struct dfa_t autocompletion_dfa_create(char *const*const commands,
  ...
  for (size_t i = 0; i < commands_count; ++i) {
    ...
    last_matching_node = dfa_insert_everything_but_last_symbol_of_token(
      last_matching_node, token);
    ...
    while (true) {
      ...
      struct dfa_node_mutable_t **const last_symbol_of_prev_word_node_ptr =
        &(last_matching_node->next[(size_t) prev_token[kPrevTokenNameLen
                                                       - 1]]);
      last_matching_node = dfa_node_mutable_append_or_create(
        last_symbol_of_prev_word_node_ptr, space_plus_token);

      last_matching_node = dfa_node_mutable_append_or_create(
        &(last_matching_node->next[(size_t) ' ']), token);
      last_matching_node = dfa_insert_everything_but_last_symbol_of_token(
        last_matching_node, token);
    }
  }

  dfa_run_callback_on_each_sibling(
    (const struct dfa_node_t* const ) &res.root,
    dfa_make_autocompletion_variants_unique);
